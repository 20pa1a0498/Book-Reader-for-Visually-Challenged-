...
SPEED   = 0.7   # Speech speed, 0.5 - 2.0
VOLUME  = 90    # Audio volume

# OTHER SETTINGS
SOUNDS  = "/home/pi/Downloads/PiTextReader-master/sounds" # Directory for sound effect(s)
...
class RaspberryThread(threading.Thread):
    def _init_(self, function):
        ...
        super(RaspberryThread, self)._init_()
...
def volume(val): # Set Volume for Launch
    ...
    vol = int(val)
    cmd = "sudo amixer -q sset PCM,0 ${VOLUME}"
    logger.info(cmd)
    os.system(cmd)
    return
...
def getData():
    ...
    # Take photo
    sound(SOUNDS+"/home/pi/Downloads/PiTextReader-master/sounds/camera-shutter.wav")
    cmd = CAMERA
    logger.info(cmd)
    os.system(cmd)
    ...
    cmd = "/home/pi/Downloads/PiTextReader-master/test.txt"
    logger.info(cmd)
    os.system(cmd)
